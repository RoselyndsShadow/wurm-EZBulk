...
public class EZBulk implements WurmClientMod, Initable, PreInitable {
    ...
    private static boolean looksLikeContainer(InventoryMetaItem item) {
        ...
        try {
            if (item.getType() == 20) return false;
        } catch (Exception ignored) {}
        if (n.contains("inventory") || n.contains("backpack")) return false;
    ...
    private static boolean tryExpandMove(long destId, InventoryMetaItem source) {
        ...
        if (!dumpAll && !isBulkCommodity(source)) {
        ...
        InventoryMetaItem dest = findItemById(destId);
        long hoverId = worldHoverId();
        String hoverName = worldHoverName();
        if ((dest == null || !looksLikeContainer(dest)) && hoverId > 0) {
            log("world hover id=" + hoverId + " name=\"" + hoverName + "\" packetDest=" + destId);
            InventoryMetaItem hoverItem = findItemById(hoverId);
            if (hoverItem != null && looksLikeContainer(hoverItem)) {
                dest = hoverItem;
                destId = hoverId;
            } else if (dest == null && destId <= 0) {
                destId = hoverId;
            } else if (dest == null) {
                destId = destId > 0 ? destId : hoverId;
            }
        }
        if (dest == null && destId <= 0 && lastPlayerTarget > 0) {
            destId = lastPlayerTarget;
            log("dest from last player action target=" + destId
                    + " arg1=" + lastPlayerArg1 + " arg2=" + lastPlayerArg2);
        }
        if (dest != null && !looksLikeContainer(dest)) {
            InventoryMetaItem destOwner = findBulkAncestor(dest);
            if (destOwner == null) destOwner = getParent(dest);
            if (destOwner != null) {
                dest = destOwner;
                destId = destOwner.getId();
            }
        }
        ...
        if (dest != null) {
            try {
                if (dest.getType() == 20 || coreName(dest).contains("inventory")) {
                    log("SHIFT drop ignored – dest is player inventory, not bulk");
                    return false;
                }
            } catch (Exception ignored) {}
        }
    ...
    private static boolean onItemDropped(Object proxy, Object[] args) {
        ...
        return tryExpandMove(destId, source);
    }

    private static void rememberShiftDrop(long destId, InventoryMetaItem source) {
        if (source == null || destId <= 0) return;
        InventoryMetaItem newDest = findItemById(destId);
        InventoryMetaItem oldDest = lastShiftDestId > 0 ? findItemById(lastShiftDestId) : null;
        if (oldDest != null && isBulkContainer(oldDest)
                && (newDest == null || !isBulkContainer(newDest))) {
            destId = lastShiftDestId;
        } else if (newDest != null && !isBulkContainer(newDest)) {
            InventoryMetaItem owner = findBulkAncestor(newDest);
            if (owner == null) owner = getParent(newDest);
            if (owner != null && isBulkContainer(owner)) destId = owner.getId();
        }
        lastShiftSource = source;
        lastShiftDestId = destId;
        sawAmountMenu = false;
        destRefused = false;
    ...
    private static boolean onHandleDrop(Object[] args) {
        ...
        log("handleDrop SHIFT dest=" + describeItem(destBulk != null ? destBulk : destItem)
    ...
    private static boolean onItemDroppedAmount(Object[] args) {
        ...
        log("itemDroppedAmount source=" + (source != null ? coreName(source) : "null") + " destId=" + destId);
        if (source == null || destId <= 0) return false;
        rememberShiftDrop(destId, source);
        return tryExpandMove(destId, source);
    }
    ...
    private static boolean onTreeItemDropped(Object proxy, Object[] args) {
        ...
        log("treeItemDropped source=" + (source != null ? coreName(source) : "null") + " destId=" + destId);
        if (source == null || destId <= 0) return false;
        rememberShiftDrop(destId, source);
        return tryExpandMove(destId, source);
    }
    ...
    private static boolean maybeSubmitRemovingItems(Object wc) {
        ...
        if (!modifier && !ourJob) return false;
        if (!modifier && suppressShiftBml) return false;
    ...
    private static boolean interceptSendMove(Object[] args) {
        ...
        if (source == null) return false;
        log("intercept sendMoveSomeItems dest=" + destId + " n=" + ids.length
                + " first=" + describeItem(source));
        rememberShiftDrop(destId, source);
        return tryExpandMove(destId, source);
    }
    ...
    private static void onBmlWindowSeen(Object wc) {
        ...
        boolean ourJob = pendingAutoAll
                || (waitingForDone && !moveQueue.isEmpty() && moveQueueIndex > 0);
        if (!modifier && !ourJob) return;
        if (!modifier && suppressShiftBml) return;
        Object titleObj = getFieldValue(wc, "title");
        String title = titleObj instanceof String ? (String) titleObj : "";
        ...
        log("BmlWindow seen title=\"" + title + "\" – auto all");
